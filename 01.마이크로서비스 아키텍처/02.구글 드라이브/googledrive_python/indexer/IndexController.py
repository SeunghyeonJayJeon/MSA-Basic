...
bp = Blueprint('index', __name__, url_prefix='/indices')

@bp.route("", methods=["POST"])
def Post():
    response = IndexRepository.Create(request)
...
def Get():
    response = IndexRepository.Read(request)
...
def Get_By_Id(id: int):
    response = IndexRepository.Read_by_id(id)
...
def Update(id: int):
    ...
    response = IndexRepository.Update(id, request)
...
def Delete(id: int):
    response = IndexRepository.Delete(id)
    if response == None:
