...
class IndexDB():
    def __init__(self):
        self.engine = create_engine('sqlite:///Index_table.db', echo=True, connect_args={'check_same_thread': False})

        Index.__table__.create(bind=self.engine, checkfirst=True)
    ...
    def find_by_id(self, id:int):
        ...
        session = DBsession()
        try:
            query = session.query(Index).filter_by(id=id).one()
